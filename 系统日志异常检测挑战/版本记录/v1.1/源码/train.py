...
def shifted_type_thresholds(type_thresholds: dict[str, float], offset: float) -> dict[str, float]:
    return {
        label: float(min(0.98, max(0.02, threshold + offset)))
        for label, threshold in type_thresholds.items()
    }
...
def tune_thresholds(train_docs, oof_doc_probs, oof_line_scores) -> tuple[dict, dict]:
    ...
    base_doc_threshold, doc_f1 = best_threshold_for_f1(y_doc, oof_doc_probs, average="macro")
    ...
    doc_candidates = sorted({float(min(0.98, max(0.02, base_doc_threshold + delta))) for delta in (-0.05, 0.0, 0.05)})
    type_offsets = [0.0]
    decoder_grid = [
        {"smoothing_window": 1, "gap_merge": 0, "min_span_len": 2, "max_span_len": 10, "max_spans": 2},
        {"smoothing_window": 1, "gap_merge": 1, "min_span_len": 2, "max_span_len": 10, "max_spans": 2},
        {"smoothing_window": 3, "gap_merge": 0, "min_span_len": 2, "max_span_len": 10, "max_spans": 2},
        {"smoothing_window": 3, "gap_merge": 1, "min_span_len": 2, "max_span_len": 10, "max_spans": 2},
        {"smoothing_window": 3, "gap_merge": 1, "min_span_len": 3, "max_span_len": 10, "max_spans": 2},
        {"smoothing_window": 3, "gap_merge": 2, "min_span_len": 3, "max_span_len": 12, "max_spans": 2},
        {"smoothing_window": 5, "gap_merge": 1, "min_span_len": 3, "max_span_len": 12, "max_spans": 2},
        {"smoothing_window": 3, "gap_merge": 1, "min_span_len": 2, "max_span_len": 10, "max_spans": 1},
    ]

    candidates = []
    for doc_threshold in doc_candidates:
        for type_offset in type_offsets:
            for decoder_config in decoder_grid:
                thresholds = {
                    "doc_threshold": float(doc_threshold),
                    "type_thresholds": shifted_type_thresholds(base_type_thresholds, type_offset),
                    "type_threshold_offset": float(type_offset),
    ...
    best_metrics = None
    for thresholds in tqdm(candidates, desc="tune decoder", unit="cfg", dynamic_ncols=True):
        predictions = decode_predictions(search_docs, search_doc_probs, search_line_scores, thresholds)
        metrics = evaluate_predictions(search_docs, predictions)
        if best_metrics is None or metrics["score"] > best_metrics["score"] + 1e-12:
            best_thresholds = thresholds
            best_metrics = metrics
        elif best_metrics is not None and abs(metrics["score"] - best_metrics["score"]) <= 1e-12:
            if metrics["iou_loc"] > best_metrics["iou_loc"]:
                best_thresholds = thresholds
                best_metrics = metrics
    ...
    full_predictions = decode_predictions(train_docs, oof_doc_probs, oof_line_scores, best_thresholds)
    full_metrics = evaluate_predictions(train_docs, full_predictions)

    report = {
        "base_doc_threshold": float(base_doc_threshold),
        "base_doc_f1": float(doc_f1),
