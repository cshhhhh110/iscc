...
def make_doc_feature_matrix(parsed_docs: Sequence[dict]) -> tuple[sp.csr_matrix, np.ndarray]:
...
def make_line_feature_matrix(
    parsed_docs: Sequence[dict], include_targets: bool
    ...
    for item in parsed_docs:
        ...
        for idx in range(n_lines):
            ...
            numeric.append(line_numeric[idx])
...
def best_fallback_span(
    ...
    if n_lines == 0:
        return None
    min_len = max(1, min(min_span_len, n_lines))
    max_len = max(min_len, min(max_span_len, n_lines))
    ...
    for type_idx, label in enumerate(ANOMALY_TYPES):
        ...
        for start in range(n_lines):
            running = 0.0
            for end in range(start, min(n_lines, start + max_len)):
                running += float(seq[end])
                length = end - start + 1
                if length < min_len:
                    continue
                score = running / float(length)
                if best is None or score > best[3]:
                    best = (start, end, label, score)
    return best
...
def decode_one_prediction(
    ...
    for type_idx, label in enumerate(ANOMALY_TYPES):
        ...
        for start, end in mask_components(mask):
            if end - start + 1 < min_span_len:
                continue
            raw_spans.append((start, end, label, float(scores[start : end + 1].mean())))
    ...
    if not spans and doc_prob >= doc_threshold:
        fallback = best_fallback_span(line_scores, smoothing_window, min_span_len, max_span_len)
