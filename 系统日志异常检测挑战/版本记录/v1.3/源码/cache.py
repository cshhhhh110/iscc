...
def build_cache(data_dir: Path) -> None:
    ...
    docs = read_documents(data_dir / "train.csv", expect_labels=True)
    ...
    joblib.dump(parsed_clean, cache_dir / "parsed_docs.joblib", compress=3)
    joblib.dump(line_contexts, cache_dir / "line_contexts.pkl", compress=3)
    joblib.dump([item["doc"].doc_id for item in parsed], cache_dir / "doc_ids.joblib", compress=3)

    print(f"Cache built successfully in {cache_dir}")
    print(f"  parsed_docs.joblib  — {len(parsed_clean)} documents")
    print(f"  line_contexts.pkl   — {len(line_contexts)} documents, {total_lines} lines")
    print(f"  doc_ids.joblib      — ID alignment")
...
if __name__ == "__main__":
    args = parse_args()
    build_cache(args.data_dir.resolve())
