...
def load_cache(data_dir: Path):
    """Load pre-computed feature cache. Returns (parsed_docs, line_contexts, id_to_idx) or None."""
    cache_dir = data_dir / "缓存"
    parsed_path = cache_dir / "parsed_docs.joblib"
    contexts_path = cache_dir / "line_contexts.pkl"
    doc_ids_path = cache_dir / "doc_ids.joblib"

    if not (parsed_path.exists() and contexts_path.exists() and doc_ids_path.exists()):
        return None

    print("Loading feature cache...")
    parsed = joblib.load(parsed_path)
    contexts = joblib.load(contexts_path)
    doc_ids = joblib.load(doc_ids_path)
    id_to_idx = {did: i for i, did in enumerate(doc_ids)}
    print(f"  {len(parsed)} documents, {sum(len(c) for c in contexts)} context strings")
    return parsed, contexts, id_to_idx


def train_one_epoch(
    docs, doc_model, type_models, doc_weights, type_weights,
    batch_size, rng, desc,
    cached_parsed=None, cached_contexts=None, id_to_idx=None,
):
    ...
    use_cache = cached_parsed is not None and cached_contexts is not None and id_to_idx is not None

    for batch_docs in tqdm(batches, desc=desc, unit="batch", dynamic_ncols=True, leave=False):
        if use_cache:
            indices = [id_to_idx[d.doc_id] for d in batch_docs]
            batch_parsed = [cached_parsed[i] for i in indices]
            batch_contexts = [cached_contexts[i] for i in indices]
            y_doc = np.array([batch_parsed[j]["has_anomaly"] for j in range(len(batch_parsed))], dtype=np.int32)
            # Reconstruct "norm_lines" from parsed cache for doc features
            doc_texts = [" <nl> ".join(item["norm_lines"]) for item in batch_parsed]
            doc_numeric = [item["doc_numeric"] for item in batch_parsed]
            from common import vectorize_text_and_numeric
            doc_matrix = vectorize_text_and_numeric(doc_texts, doc_numeric)
        else:
        ...
        if use_cache:
            ...
            line_numeric = []
            line_lengths = []
            for j in range(len(batch_parsed)):
                ctx = batch_contexts[j]
                line_texts.extend(ctx)
                line_numeric.extend(batch_parsed[j]["line_numeric"])
                line_lengths.append(len(ctx))
            from common import vectorize_text_and_numeric, line_targets_for_doc
            line_matrix = vectorize_text_and_numeric(line_texts, line_numeric)
            line_targets = None
            for j, d in enumerate(batch_docs):
                t = line_targets_for_doc(d, line_lengths[j])
                if line_targets is None:
                    line_targets = {label: [] for label in ANOMALY_TYPES}
                for label in ANOMALY_TYPES:
                    line_targets[label].append(t[label])
        else:
            parsed = parse_documents_batch(batch_docs)
        ...
        for label, model in type_models.items():
            y_type = np.concatenate(line_targets[label]) if isinstance(line_targets[label], list) else line_targets[label]
            y_type = y_type.astype(np.int32)
...
def train_fold_sgd(train_docs, val_docs, args, fold_idx: int, cache_data=None):
    ...
    for epoch in range(1, args.epochs + 1):
        train_one_epoch(
            ...
            cached_parsed=cached_parsed, cached_contexts=cached_contexts, id_to_idx=id_to_idx,
        )
...
def train_full_model_sgd(train_docs, args, epochs: int, cache_data=None):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-cache", action="store_true", default=False, help="Ignore feature cache.")
...
def main() -> int:
    ...
    append_action_log(args.action_log, "Training started for system log anomaly detection (v1.3 SGD).")
    ...
    if not args.no_cache:
        cache_data = load_cache(data_dir)
        if cache_data:
            print("Using feature cache — skipping document parsing and context building")
        else:
            print("No feature cache found. Run 'python 源码/cache.py' first to build it.")
            print("Training will proceed without cache (slower).")
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(
        ...
        result = train_fold_sgd(train_fold_docs, val_fold_docs, args, fold_idx, cache_data=cache_data)
    ...
    doc_model, type_models = train_full_model_sgd(train_docs, args, epochs=final_epochs, cache_data=cache_data)
    bundle = {
        "version": 3,
        "model": "SGD",
        "seed": args.seed,
