...
Write-Host "===== Train NN v2 $(Get-Date -Format 'HH:mm:ss') ====="
& $PY (Join-Path $SRC "train_nn_v2.py")
if ($LASTEXITCODE -ne 0) { Write-Host "TRAIN FAILED (exit code: $LASTEXITCODE)"; Read-Host "Press Enter"; exit 1 }

Write-Host "===== Predict NN v2 $(Get-Date -Format 'HH:mm:ss') ====="
& $PY (Join-Path $SRC "predict_nn_v2.py")
