...
def decode_one_prediction(
    ...
    # Boundary refinement: try ±1 expansion/shrink for each span
    refined_spans: list[tuple[int, int, str, float]] = []
    for s, e, label, score in spans:
        type_idx = TYPE_TO_INDEX[label]
        scores = line_scores[:, type_idx]
        best_s, best_e, best_score = s, e, score
        for ds in (-1, 0, 1):
            for de in (-1, 0, 1):
                ns, ne = max(0, s + ds), min(n_lines - 1, e + de)
                if ns > ne or ns < 0 or ne >= n_lines:
                    continue
                length = ne - ns + 1
                new_score = float(scores[ns:ne + 1].mean())
                label_min, label_max, target_len = _span_length_profile(label, min_span_len, max_span_len, n_lines)
                if length < label_min or length > label_max:
                    continue
                new_score -= 0.005 * abs(length - target_len) / max(1, target_len)
                if new_score > best_score:
                    best_s, best_e, best_score = ns, ne, new_score
        refined_spans.append((best_s, best_e, label, best_score))

    spans = sorted(refined_spans, key=lambda item: (item[0], -item[3], item[1], item[2]))[:max_spans]
    if not spans:
        return Prediction(str(doc.doc_id), 0, -1, -1, "none", "")
    primary = spans[0]
