"""Training entrypoint for the ISCC binary vulnerability v1.1 pipeline."""
...
from models import (
    ...
    FUSION_CONFIG_NAME,
    LEGACY_CWE_MAPPING_NAME,
    LEGACY_CWE_MODEL_NAME,
    LEGACY_FEATURE_COLUMNS_NAME,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the v1.1 ISCC competition baseline.")
    parser.add_argument("--epochs", type=int, default=6, help="Neural training epochs.")
    parser.add_argument("--batch-size", type=int, default=64, help="Training batch size.")
    parser.add_argument("--byte-length", type=int, default=DEFAULT_BYTE_LENGTH, help="Fixed byte-window length.")
    parser.add_argument("--lr", type=float, default=1e-3, help="Neural learning rate.")
    parser.add_argument("--dropout", type=float, default=0.25, help="Neural dropout rate.")
    parser.add_argument("--retrain-tree", action="store_true", help="Force retraining of the legacy tree baseline.")
...
def _search_binary_fusion(y_true: np.ndarray, tree_probs: np.ndarray, neural_probs: np.ndarray) -> Tuple[float, float, float]:
    ...
    best_score = -1.0
    best_acc = -1.0
    for neural_weight in np.linspace(0.0, 1.0, 21):
...
def _train_legacy_tree_models(
    X: np.ndarray,
    y_label: np.ndarray,
    cwe_ids: Sequence[str],
    force_retrain: bool,
) -> Tuple[object, float, object, List[str]]:
    label_path = MODEL_DIR / LEGACY_LABEL_MODEL_NAME
    cwe_path = MODEL_DIR / LEGACY_CWE_MODEL_NAME
    ...
    label_model = HistGradientBoostingClassifier(**label_params)
    ...
    class_weights = np.bincount(y_cwe, minlength=len(classes)).astype(np.float32)
    class_weights[class_weights <= 0] = 1.0
    class_weights = class_weights.sum() / (len(class_weights) * class_weights)
    class_weight_map = {index: float(weight) for index, weight in enumerate(np.clip(class_weights, 0.25, 20.0))}
    ...
    joblib.dump(
        {"model": label_model, "threshold": threshold, "feature_columns": get_feature_columns()},
        label_path,
    )
    joblib.dump(
        {"model": cwe_model, "feature_columns": get_feature_columns(), "classes": classes},
        cwe_path,
    )
...
def _build_neural_model(
    ...
    return ByteMetaMultiTaskNet(tabular_dim=tabular_dim, num_cwe_classes=num_cwe_classes, dropout=dropout)
...
def _train_neural_model(
    ...
    model = _build_neural_model(tabular_dim=X_train_tab.shape[1], num_cwe_classes=num_cwe_classes, dropout=dropout).to(DEVICE)
    ...
    for epoch in range(1, epochs + 1):
        ...
        composite_score = neural_label_f1 + 0.3 * neural_cwe_macro
...
def main() -> None:
    ...
    with tqdm(total=5, desc="Training pipeline", unit="stage") as pipeline:
        ...
        pipeline.set_postfix_str("tree baseline")
        legacy_tree_available = (MODEL_DIR / LEGACY_LABEL_MODEL_NAME).exists() and (MODEL_DIR / LEGACY_CWE_MODEL_NAME).exists()
        label_model, tree_threshold, cwe_model, cwe_classes = _train_legacy_tree_models(
            X,
            y_label,
            cwe_ids,
        ...
        joblib.dump(
            {
                "label_model_file": LEGACY_LABEL_MODEL_NAME,
                "cwe_model_file": LEGACY_CWE_MODEL_NAME,
                "feature_columns": feature_columns,
                "cwe_classes": cwe_classes,
                "tree_threshold": float(tree_threshold),
                "tree_model_source": "legacy" if legacy_tree_available and not args.retrain_tree else "trained",
            },
