"""v2.0 training: homogeneous emb=48 + self-attention + rare CWE augment + SWA."""
...
def main():
    ...
    seeds = args.seeds
    print(f"v2.0 ensemble training: seeds={seeds}, device={DEVICE}")
    ...
    # ---- Rare CWE augmentation (v2.0) ----
    # Duplicate samples from rare CWE classes with small perturbations
    pos_y_cwe = y_cwe[y_cwe >= 0]
    cwe_counts = np.bincount(pos_y_cwe, minlength=len(cwe_classes))
    rare_classes = [c for c in range(len(cwe_classes)) if 0 < cwe_counts[c] <= 5]
    if rare_classes:
        aug_bytes = []
        aug_tab = []
        aug_labels = []
        ...
        for c in rare_classes:
            c_mask = y_cwe == c
            n_orig = int(c_mask.sum())
            n_aug = max(3, 10 - n_orig)  # boost to at least 10 samples
            c_indices = np.where(c_mask)[0]
            for _ in range(n_aug):
                idx = int(rng.choice(c_indices))
                # Augment byte data: flip ~1% of bytes
                aug_byte = X_byte[idx].copy()
                flip_mask = rng.random(aug_byte.shape) < 0.01
                aug_byte[flip_mask] = rng.integers(0, 256, size=int(flip_mask.sum()), dtype=np.uint8)
                # Augment tabular data: Gaussian noise σ=0.01
                aug_tabular = X[idx].copy() + rng.normal(0, 0.01, size=X.shape[1]).astype(np.float32)
                aug_bytes.append(aug_byte)
                aug_tab.append(aug_tabular)
                aug_labels.append(y_label[idx])
                aug_cwe.append(c)
        if aug_bytes:
            X_byte_aug = np.stack(aug_bytes, axis=0)
            X_aug = np.stack(aug_tab, axis=0)
            y_label_aug = np.array(aug_labels, dtype=np.int32)
            y_cwe_aug = np.array(aug_cwe, dtype=np.int32)
            cwe_ids_aug = [cwe_classes[c] for c in aug_cwe]
            X_byte = np.concatenate([X_byte, X_byte_aug], axis=0)
            X = np.concatenate([X, X_aug], axis=0)
            y_label = np.concatenate([y_label, y_label_aug], axis=0)
            y_cwe = np.concatenate([y_cwe, y_cwe_aug], axis=0)
            cwe_ids.extend(cwe_ids_aug)
            print(f"Rare CWE augment: {len(rare_classes)} classes ({', '.join(cwe_classes[c] for c in rare_classes)}), "
                  f"+{len(aug_bytes)} samples (total now {len(y_label)})")
    ...
    for seed in seeds:
        ...
        torch.save({
            ...
            "byte_length": int(args.byte_length), "metrics": neural_metrics, "model_version": "v2.0", "seed": seed,
    ...
    joblib.dump({
        ...
        "model_version": "v2.0", "label_model_files": {s: seed_label_model(s) for s in seeds},
        "cwe_model_files": {s: seed_cwe_model(s) for s in seeds},
    }, MODEL_DIR / TABULAR_BUNDLE_NAME)

    print(f"\n{'='*60}")
    print(f"v2.0 training complete!")
    print(f"Fusion mode: scalar + self-attention + SWA + cosine annealing")
