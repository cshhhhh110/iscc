"""Prediction entrypoint for v1.5: ensemble + TTA."""
...
from models import (
    ...
    TEST_BYTE_CACHE_NAME, TEST_CACHE_NAME,
    ensure_model_dir, seed_label_model, seed_cwe_model,
    seed_neural_bundle, seed_fusion_mlp,
)
from nn_models import (
    ByteMetaMultiTaskNet, FusionMLP, TabularNormalizer,
    apply_tabular_normalizer, predict_fusion_mlp, predict_multitask,
)
...
def main():
    ...
    print(f"v1.5 prediction: fusion_mode={fusion_mode}, seeds={seeds}, tta={use_tta}({tta_windows} windows)")
    ...
    for seed in seeds:
        ...
        # MLP fusion (if available)
        fusion_path = MODEL_DIR / seed_fusion_mlp(seed)
        if fusion_mode == "mlp" and fusion_path.exists():
            fusion_bundle = _torch_load(fusion_path, DEVICE)
            fusion_mlp = FusionMLP(num_cwe_classes=len(cwe_classes)).to(DEVICE)
            fusion_mlp.load_state_dict(fusion_bundle["state_dict"])
            fusion_mlp.eval()
            flp, fcp = predict_fusion_mlp(
                fusion_mlp, tree_lp, tree_cp, neural_lp, neural_cp,
                batch_size=batch_size, device=DEVICE)
            all_fusion_label.append(flp)
            all_fusion_cwe.append(fcp)

    # Average across seeds
    avg_tree_label = np.mean(all_tree_label, axis=0).astype(np.float32)
    avg_tree_cwe = np.mean(all_tree_cwe, axis=0).astype(np.float32)
    avg_neural_label = np.mean(all_neural_label, axis=0).astype(np.float32)
    avg_neural_cwe = np.mean(all_neural_cwe, axis=0).astype(np.float32)

    if all_fusion_label:
        avg_fusion_label = np.mean(all_fusion_label, axis=0).astype(np.float32)
        avg_fusion_cwe = np.mean(all_fusion_cwe, axis=0).astype(np.float32)
        print(f"\nUsing averaged MLP fusion predictions ({len(all_fusion_label)} seeds)")
        label_probs = avg_fusion_label
        cwe_probs = avg_fusion_cwe
        fusion_threshold = float(np.mean([fusion_config["seed_results"][str(s)].get("mlp_threshold", 0.5) for s in seeds]))
    else:
        print(f"\nUsing scalar fusion (average of {len(seeds)} seeds)")
