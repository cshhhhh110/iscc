"""v1.5 training: pseudo-labeling + 3-seed ensemble + proper MLP fusion + TTA-ready."""
...
def _load_or_build_byte_cache(rows: List[Dict[str, str]], byte_length: int) -> Dict[str, object]:
    ...
    if cache_path.exists():
        ...
        if cached_length == byte_length and getattr(cached_matrix, "shape", (0, 0))[1] == byte_length:
            return cache
        print("warning: byte cache mismatch; rebuilding.")
...
def _pseudo_label_test_set(args: argparse.Namespace) -> List[Dict[str, str]]:
    """Use v1.4 models to generate high-confidence pseudo-labels for test.csv."""
    ...
    print("=== Phase 1: Pseudo-labeling test.csv with v1.4 models ===")
    # Load v1.4 feature caches
    v4_test_cache = MODEL_DIR / "test_features_v1.4.joblib"
    v4_byte_cache = MODEL_DIR / "test_bytes_v1.4.joblib"
    v4_neural = MODEL_DIR / "neural_bundle_v1.4.pt"
    v4_tabular = MODEL_DIR / "tabular_bundle_v1.4.joblib"
    v4_cwe = MODEL_DIR / "cwe_model_v1.4.joblib"
    v4_label = MODEL_DIR / "label_model_v1.4.joblib"
    v4_fusion_config = MODEL_DIR / "fusion_config_v1.4.json"

    if not v4_test_cache.exists():
        print("Building v1.4 test tabular cache...")
        test_rows = read_csv_rows(TEST_CSV)
        X_test, _, _, test_binary_ids = _rows_to_matrix(test_rows)
        joblib.dump({"X": X_test, "binary_ids": test_binary_ids}, v4_test_cache)
    else:
        cache = joblib.load(v4_test_cache)
        X_test = np.asarray(cache["X"], dtype=np.float32)
        test_binary_ids = list(cache["binary_ids"])
        test_rows = read_csv_rows(TEST_CSV)

    if not v4_byte_cache.exists():
        print("Building v1.4 test byte cache...")
        Xb, _ = rows_to_byte_matrix(test_rows, BINARIES_DIR, byte_length=8192, desc="Test bytes v1.4")
        joblib.dump({"X_byte": Xb, "binary_ids": test_binary_ids, "byte_length": 8192}, v4_byte_cache)
    else:
        Xb = np.asarray(joblib.load(v4_byte_cache)["X_byte"], dtype=np.uint8)

    # Load v1.4 models
    label_bundle = joblib.load(v4_label)
    cwe_bundle = joblib.load(v4_cwe)
    tab_bundle = joblib.load(v4_tabular)
    cwe_classes = list(tab_bundle["cwe_classes"])
    label_model = label_bundle["model"]
    cwe_model = cwe_bundle["model"]

    neural_bundle = torch.load(v4_neural, map_location=DEVICE, weights_only=False)
    ...
    fusion_config = _read_json(v4_fusion_config)

    # Predict
    tree_label_probs = _aligned_positive_probability_v4(label_model, X_test)
    tree_cwe_probs = _aligned_cwe_probability_v4(cwe_model, X_test, len(cwe_classes))
    X_neural = apply_tabular_normalizer(X_test, normalizer)
    neural_label_probs, neural_cwe_probs = predict_multitask(
        neural_model, Xb, X_neural, batch_size=128, device=DEVICE, desc="Pseudo-label predict",
    )

    # Scalar fusion (use v1.4 config weights)
    lw_n = float(fusion_config.get("scalar_neural_label_weight", 0.475))
    lw_t = float(fusion_config.get("scalar_tree_label_weight", 0.525))
    cw_n = float(fusion_config.get("scalar_neural_cwe_weight", 0.525))
    cw_t = float(fusion_config.get("scalar_tree_cwe_weight", 0.475))
    threshold = float(fusion_config.get("fusion_threshold", 0.5))

    label_probs = lw_n * neural_label_probs + lw_t * tree_label_probs
    cwe_probs = cw_n * neural_cwe_probs + cw_t * tree_cwe_probs
    ...
    for i, bid in enumerate(tqdm(test_binary_ids, desc="Filtering pseudo-labels")):
        ...
        if lp >= PSEUDO_LABEL_THRESH_HIGH:
            ...
            if cwe_max >= PSEUDO_CWE_THRESH:
                ...
                continue  # skip if CWE confidence too low
        elif lp <= PSEUDO_LABEL_THRESH_LOW:
            pred_label = "0"
            pred_cwe = ""
        else:
            continue  # skip uncertain predictions
        pseudo_rows.append({"binary_id": bid, "label": pred_label, "cwe_id": pred_cwe})
...
def _train_neural_one_seed(X_byte, X_tab, y_label, y_cwe, train_idx, val_idx,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"  epoch {epoch}: label_f1={neural_lf1:.4f} cwe_macro={neural_cm:.4f} composite={composite:.4f}")
...
def main():
    ...
    seeds = args.seeds
    print(f"v1.5 ensemble training: seeds={seeds}, device={DEVICE}")
    ...
    for seed in seeds:
        ...
        torch.save({
            ...
            "model_config": {"tabular_dim": int(X.shape[1]), "num_cwe_classes": int(len(cwe_classes)),
                             "dropout": float(args.dropout), "byte_embedding_dim": int(args.byte_embedding_dim)},
            ...
            "byte_length": int(args.byte_length), "metrics": neural_metrics, "model_version": "v1.5", "seed": seed,
        ...
        # MLP fusion with proper split
        tree_cp_val = np.zeros((len(val_idx), len(cwe_classes)), dtype=np.float32)
        tree_cp_val[pos_mask_val] = tree_cp_full[pos_mask_val]
        fusion_model, f_lf1, f_cm, fusion_metrics = _train_fusion_mlp_proper(
            tree_lp, tree_cp_val, neural_lp, neural_cp,
            y_label[val_idx], y_cwe[val_idx], len(cwe_classes), seed, args)

        torch.save({
            "state_dict": {k: v.detach().cpu() for k, v in fusion_model.state_dict().items()},
            "num_cwe_classes": len(cwe_classes), "model_version": "v1.5", "seed": seed,
        }, MODEL_DIR / seed_fusion_mlp(seed))
        ...
        seed_results[seed] = {
            ...
            "mlp_label_f1": float(fusion_metrics["heldout_label_f1"]),
            "mlp_cwe_macro": float(fusion_metrics["heldout_cwe_macro"]),
            "mlp_threshold": float(fusion_metrics["threshold"]),
            "neural_label_f1": float(neural_metrics["label_f1"]),
            "neural_cwe_macro": float(neural_metrics["cwe_macro_f1"]),
        }
        print(f"Seed {seed} done: scalar_cwe={s_cm:.4f}, mlp_cwe(heldout)={fusion_metrics['heldout_cwe_macro']:.4f}")

    # ---- Phase 4: Save ensemble config ----
    print("\n=== Phase 4: Saving ensemble config ===")
    # Determine best fusion mode
    avg_scalar = np.mean([r["scalar_cwe_macro"] for r in seed_results.values()])
    avg_mlp = np.mean([r["mlp_cwe_macro"] for r in seed_results.values()])
    best_mode = "mlp" if avg_mlp > avg_scalar else "scalar"
    print(f"Average: scalar_cwe={avg_scalar:.4f}, mlp_cwe={avg_mlp:.4f} → best_mode={best_mode}")

    # Average scalar weights
    avg_nw_label = np.mean([r["scalar_neural_label_weight"] for r in seed_results.values()])
    avg_nw_cwe = np.mean([r["scalar_neural_cwe_weight"] for r in seed_results.values()])
    ...
    ensemble_config = {
        ...
        # Average scalar weights
        "scalar_neural_label_weight": float(avg_nw_label),
        "scalar_tree_label_weight": float(1.0 - avg_nw_label),
        "scalar_neural_cwe_weight": float(avg_nw_cwe),
        "scalar_tree_cwe_weight": float(1.0 - avg_nw_cwe),
        ...
        "avg_mlp_cwe_macro": float(avg_mlp),
    ...
    joblib.dump({
        ...
        "model_version": "v1.5", "label_model_files": {s: seed_label_model(s) for s in seeds},
        "cwe_model_files": {s: seed_cwe_model(s) for s in seeds},
    }, MODEL_DIR / TABULAR_BUNDLE_NAME)

    print(f"\n{'='*60}")
    print(f"v1.5 training complete!")
    print(f"Best fusion mode: {best_mode}")
    print(f"Avg scalar CWE macro: {avg_scalar:.4f}")
    print(f"Avg MLP CWE macro:   {avg_mlp:.4f}")
