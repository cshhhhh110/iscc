"""Prediction entrypoint for v1.7: weighted ensemble + per-CWE thresholds + TTA."""
...
def main():
    ...
    # Per-CWE thresholds: for each positive prediction, check against class-specific threshold
    per_cwe_thresholds = fusion_config.get("per_cwe_thresholds", {})

    # Generate submission
    label_pred = (label_probs >= fusion_threshold).astype(int)
    cwe_pred = [""] * len(rows)
    cwe_used_per_class = 0
    cwe_used_global = 0
    for idx in np.where(label_pred == 1)[0]:
        cwe_cls = int(cwe_probs[idx].argmax())
        cls_thresh = per_cwe_thresholds.get(str(cwe_cls), 0.5) if per_cwe_thresholds else fusion_threshold
        if per_cwe_thresholds:
            cwe_used_per_class += 1
            if cwe_probs[idx, cwe_cls] >= cls_thresh:
                cwe_pred[idx] = cwe_classes[cwe_cls]
            else:
                label_pred[idx] = 0  # demote uncertain CWE
        else:
            cwe_used_global += 1
            cwe_pred[idx] = cwe_classes[cwe_cls]

    if cwe_used_per_class > 0:
        print(f"  Per-CWE thresholds applied to {cwe_used_per_class} samples")
