"""v1.8 training: v1.5 pseudo-labels + v1.6 architecture + weighted ensemble (no per-CWE thresholds)."""
...
def main():
    ...
    seeds = args.seeds
    print(f"v1.8 ensemble training: seeds={seeds}, device={DEVICE}")
    ...
    joblib.dump({
        ...
        "model_version": "v1.8", "label_model_files": {s: seed_label_model(s) for s in seeds},
        "cwe_model_files": {s: seed_cwe_model(s) for s in seeds},
    }, MODEL_DIR / TABULAR_BUNDLE_NAME)

    print(f"\n{'='*60}")
    print(f"v1.8 training complete!")
    print(f"Fusion mode: scalar (weighted ensemble)")
