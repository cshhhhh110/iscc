"""v1.7 training: v1.5 pseudo-labels + v1.6 architecture + per-CWE thresholds + weighted ensemble."""
...
# Heterogeneous configs per seed for ensemble diversity
SEED_CONFIGS = {
    42:  {"byte_embedding_dim": 48, "dropout": 0.22},
    123: {"byte_embedding_dim": 64, "dropout": 0.26},
    456: {"byte_embedding_dim": 48, "dropout": 0.18},
    789: {"byte_embedding_dim": 56, "dropout": 0.24},
}
...
def _load_or_build_tabular_cache(rows: List[Dict[str, str]]) -> Dict[str, object]:
    ...
    # Try to reuse an existing cache with matching binary_ids
    ...
    if reused:
        ...
        # Save as current version for future fast path
...
def main():
    ...
    seeds = args.seeds
    print(f"v1.7 ensemble training: seeds={seeds}, device={DEVICE}")
    ...
    for seed in seeds:
        ...
        # Neural — use heterogeneous config per seed
        seed_cfg = SEED_CONFIGS.get(seed, {"byte_embedding_dim": args.byte_embedding_dim, "dropout": args.dropout})
        old_emb = args.byte_embedding_dim
        old_drop = args.dropout
        args.byte_embedding_dim = seed_cfg["byte_embedding_dim"]
        args.dropout = seed_cfg["dropout"]
        ...
        torch.save({
            ...
            "model_config": {"tabular_dim": int(X.shape[1]), "num_cwe_classes": int(len(cwe_classes)),
                             "dropout": seed_cfg["dropout"], "byte_embedding_dim": seed_cfg["byte_embedding_dim"]},
            ...
            "byte_length": int(args.byte_length), "metrics": neural_metrics, "model_version": "v1.6", "seed": seed,
        }, MODEL_DIR / seed_neural_bundle(seed))

        args.byte_embedding_dim = old_emb
        args.dropout = old_drop
        ...
        print(f"Scalar fusion seed={seed}: label_f1={s_lf1:.4f} cwe_macro={s_cm:.4f} "
              f"(emb={seed_cfg['byte_embedding_dim']}, drop={seed_cfg['dropout']})")

        # Per-CWE threshold search on val set
        fused_cwe_for_thresh = nw_cwe * neural_cp + (1.0 - nw_cwe) * tree_cp_full
        per_cwe_thresholds = {}
        for c in range(len(cwe_classes)):
            c_mask = y_cwe[val_idx] == c
            if c_mask.sum() > 0:
                best_t, best_f1 = 0.5, -1.0
                for t in np.arange(0.05, 0.96, 0.025):
                    pred = fused_cwe_for_thresh[:, c] >= t
                    tp = (pred & c_mask).sum()
                    fp = (pred & ~c_mask).sum()
                    fn = (~pred & c_mask).sum()
                    precision = tp / max(tp + fp, 1)
                    recall = tp / max(tp + fn, 1)
                    f1_c = 2 * precision * recall / max(precision + recall, 1e-12)
                    if f1_c > best_f1:
                        best_f1, best_t = f1_c, float(t)
                per_cwe_thresholds[str(c)] = best_t
            else:
                per_cwe_thresholds[str(c)] = 0.5
        ...
        seed_results[seed] = {
            ...
            "byte_embedding_dim": seed_cfg["byte_embedding_dim"],
            "dropout": seed_cfg["dropout"],
            "per_cwe_thresholds": per_cwe_thresholds,
        }
    ...
    # Average per-CWE thresholds across seeds
    avg_per_cwe_thresholds = {}
    for c in range(len(cwe_classes)):
        thresholds_c = [seed_results[s]["per_cwe_thresholds"][str(c)] for s in seeds]
        avg_per_cwe_thresholds[str(c)] = float(np.mean(thresholds_c))
    ...
    ensemble_config = {
        ...
        # Weighted scalar fusion weights
        ...
        # Per-CWE thresholds
        "per_cwe_thresholds": avg_per_cwe_thresholds,
        # Per-seed details
    ...
    print(f"\n{'='*60}")
    print(f"v1.7 training complete!")
    print(f"Fusion mode: scalar (weighted ensemble + per-CWE thresholds)")
