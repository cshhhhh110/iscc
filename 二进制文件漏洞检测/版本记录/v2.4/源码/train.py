"""v2.4 training: v2.3 + extended Capstone (regs + functions + operands + instr length)."""
...
def main():
    ...
    for seed in seeds:
        ...
        torch.save({
            ...
            "byte_length": int(args.byte_length), "metrics": neural_metrics, "model_version": "v2.4", "seed": seed,
    ...
    joblib.dump({
        ...
        "model_version": "v2.4", "label_model_files": {s: seed_label_model(s) for s in seeds},
        "cwe_model_files": {s: seed_cwe_model(s) for s in seeds},
    }, MODEL_DIR / TABULAR_BUNDLE_NAME)

    print(f"\n{'='*60}")
    print(f"v2.4 training complete!")
    print(f"Fusion mode: scalar + SWA + cosine + Capstone extended")
