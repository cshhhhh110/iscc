"""Training entrypoint for the ISCC binary vulnerability v1.4 pipeline.

v1.4 improvements over v1.3:
- Focal Loss replaces CrossEntropyLoss for the CWE head
- CatBoost/LightGBM replaces RF+ET VotingClassifier for the CWE tabular model
- WeightedRandomSampler ensures balanced CWE batches
- Deeper byte encoder (embedding 48, 5 conv blocks, wider MLP)
- MLP meta-learner fusion replaces scalar-weighted fusion
"""
...
def _detect_gbdt_backend() -> str:
    ...
    for name, module in [("lightgbm", "lightgbm"), ("catboost", "catboost")]:
        try:
            ...
            continue
    raise ImportError("Neither catboost nor lightgbm is installed. pip install catboost or lightgbm")
...
def _build_cwe_gbdt(num_classes: int, class_weight_map: Dict[int, float], random_state: int):
    ...
    if backend == "catboost":
        ...
        return CatBoostClassifier(
            ...
            class_weights=list(sample_weight_array),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the v1.4 ISCC competition baseline.")
    parser.add_argument("--epochs", type=int, default=25, help="Neural training epochs.")
    parser.add_argument("--batch-size", type=int, default=64, help="Training batch size.")
    parser.add_argument("--byte-length", type=int, default=8192, help="Fixed byte-window length.")
    parser.add_argument("--lr", type=float, default=7e-4, help="Neural learning rate.")
    parser.add_argument("--dropout", type=float, default=0.22, help="Neural dropout rate.")
    parser.add_argument("--byte-embedding-dim", type=int, default=48, help="Byte embedding width for the neural branch.")
    parser.add_argument("--patience", type=int, default=6, help="Early stopping patience.")
    parser.add_argument("--cwe-loss-weight", type=float, default=2.0, help="Loss weight for the CWE head.")
    parser.add_argument("--focal-gamma", type=float, default=1.5, help="Gamma for Focal Loss.")
    parser.add_argument("--fusion-epochs", type=int, default=25, help="Epochs for MLP fusion training.")
...
def _train_tabular_ensemble(
    ...
    force_retrain: bool,
) -> Tuple[object, float, object, List[str], object, object]:
    ...
    # Label model: keep HGB+ET voting (already strong)
    label_model_eval = _build_label_ensemble(random_state=42)
    label_model_eval.fit(X_tr, y_tr)
    threshold = _best_threshold(y_val, label_model_eval.predict_proba(X_val)[:, 1])
    label_model = _build_label_ensemble(random_state=42)
    label_model.fit(X, y_label)

    # CWE model: replace RF+ET with CatBoost/LightGBM
    ...
    for index, cwe_id in enumerate(cwe_ids):
        if cwe_id:
            full_y_cwe[index] = mapping[cwe_id]

    train_pos_mask = y_label[train_idx] == 1
    cwe_model_eval = _build_cwe_gbdt(
        num_classes=len(classes),
        class_weight_map=class_weight_map,
        random_state=42,
    )
    if np.any(train_pos_mask):
        cwe_model_eval.fit(X_tr[train_pos_mask], full_y_cwe[train_idx][train_pos_mask])
    else:
        cwe_model_eval.fit(X[positive_mask], y_cwe)

    cwe_model = _build_cwe_gbdt(
        num_classes=len(classes),
        class_weight_map=class_weight_map,
        random_state=42,
    )
    ...
    joblib.dump(
        {
            ...
            "feature_columns": get_feature_columns(),
            "classes": classes,
            "model_family": _detect_gbdt_backend(),
            "class_weight_map": class_weight_map,
        },
        cwe_path,
    )

    return label_model, threshold, cwe_model, classes, label_model_eval, cwe_model_eval
...
def _train_neural_model(
    ...
    # Label loss: BCEWithLogitsLoss with pos_weight
    ...
    # CWE loss: Focal Loss with class weights as alpha
    cwe_weights = build_cwe_class_weights(y_cwe[train_idx], num_cwe_classes)
    cwe_criterion = FocalLoss(gamma=focal_gamma, alpha=cwe_weights)
    cwe_criterion = cwe_criterion.to(DEVICE)
    ...
    for epoch in range(1, epochs + 1):
        ...
        total_batches = int(np.ceil(len(train_idx) / batch_size))
        ...
        print(f"  epoch {epoch}: label_f1={neural_label_f1:.4f} cwe_macro={neural_cwe_macro:.4f} composite={composite_score:.4f} lr={optimizer.param_groups[0]['lr']:.2e}")
...
def _train_fusion_mlp(
    tree_label_probs: np.ndarray,
    tree_cwe_probs: np.ndarray,
    neural_label_probs: np.ndarray,
    neural_cwe_probs: np.ndarray,
    y_label: np.ndarray,
    y_cwe: np.ndarray,
    num_cwe_classes: int,
    epochs: int,
    batch_size: int,
) -> Tuple[FusionMLP, float, float, Dict[str, float]]:
    """Train an MLP meta-learner to fuse tree and neural probabilities."""
    X_input = np.concatenate(
        [
            tree_label_probs.reshape(-1, 1),
            tree_cwe_probs,
            neural_label_probs.reshape(-1, 1),
            neural_cwe_probs,
        ],
        axis=1,
    ).astype(np.float32)

    # Use the full val set for training; evaluate on the same set (early stopping prevents overfitting)
    n = len(X_input)
    train_idx = np.arange(n)
    val_idx = np.arange(n)  # eval on full set for monitoring
    ...
    cwe_weights = build_cwe_class_weights(y_cwe, num_cwe_classes)
    ...
    for epoch in range(1, epochs + 1):
        fusion_model.train()
        perm = np.random.default_rng(42 + epoch).permutation(len(train_idx))
        total_loss = 0.0
        total_batches = max(1, int(np.ceil(len(train_idx) / batch_size)))
        with tqdm(total=total_batches, desc=f"Fusion epoch {epoch}/{epochs}", unit="batch") as progress:
            for batch_no, start in enumerate(range(0, len(train_idx), batch_size), 1):
                batch_idx = train_idx[perm[start : start + batch_size]]
                x_batch = torch.from_numpy(X_input[batch_idx]).to(DEVICE)
                label_batch = torch.from_numpy(y_label[batch_idx].astype(np.float32)).to(DEVICE)
                cwe_batch = torch.from_numpy(y_cwe[batch_idx].astype(np.int64)).to(DEVICE)

                optimizer.zero_grad(set_to_none=True)
                label_logits, cwe_logits = fusion_model(x_batch)
                label_loss = label_criterion(label_logits, label_batch)
                pos_mask = cwe_batch >= 0
                if pos_mask.any():
                    cwe_loss = cwe_criterion(cwe_logits[pos_mask], cwe_batch[pos_mask])
                else:
                    cwe_loss = torch.zeros((), device=DEVICE)
                loss = label_loss + cwe_loss
                loss.backward()
        ...
        fusion_label_probs, fusion_cwe_probs = predict_fusion_mlp(
            fusion_model,
            tree_label_probs[val_idx],
            tree_cwe_probs[val_idx],
            neural_label_probs[val_idx],
            neural_cwe_probs[val_idx],
            batch_size=batch_size,
            device=DEVICE,
        )
        fusion_thresh = _best_threshold(y_label[val_idx], fusion_label_probs)
        fusion_label_pred = (fusion_label_probs >= fusion_thresh).astype(int)
        fusion_label_f1 = f1_score(y_label[val_idx], fusion_label_pred)

        val_pos = y_label[val_idx] == 1
        if val_pos.any():
            fusion_cwe_pred = fusion_cwe_probs[val_pos].argmax(axis=1)
            fusion_cwe_macro = f1_score(
                y_cwe[val_idx][val_pos],
                fusion_cwe_pred,
                average="macro",
                labels=list(range(num_cwe_classes)),
                zero_division=0,
            ...
            fusion_cwe_macro = 0.0

        composite = fusion_label_f1 + 0.45 * fusion_cwe_macro
        print(f"  fusion epoch {epoch}: label_f1={fusion_label_f1:.4f} cwe_macro={fusion_cwe_macro:.4f} composite={composite:.4f} loss={total_loss / total_batches:.4f}")
        prev_cwe = best_metrics.get("cwe_macro_f1", 0.0)
        if composite > best_composite:
            best_composite = composite
            best_state = {k: v.detach().cpu() for k, v in fusion_model.state_dict().items()}
            best_metrics = {
                "label_f1": float(fusion_label_f1),
                "cwe_macro_f1": float(fusion_cwe_macro),
                "threshold": float(fusion_thresh),
                "epoch": epoch,
            }
            patience_counter = 0
        else:
            # Only count as patience if cwe_macro doesn't improve by > 0.001
            if fusion_cwe_macro <= prev_cwe + 0.001:
                patience_counter += 1
            else:
                patience_counter = 0
    ...
    return fusion_model, fusion_label_f1, fusion_cwe_macro, best_metrics
...
def main() -> None:
    ...
    with tqdm(total=6, desc="Training pipeline v1.4", unit="stage") as pipeline:
        ...
        _label_model_final, tree_threshold, _cwe_model_final, cwe_classes, label_model_eval, cwe_model_eval = _train_tabular_ensemble(
            X, y_label, cwe_ids, force_retrain=args.retrain_tree,
        )
        ...
        pipeline.set_postfix_str("fusion validation")
        tree_label_probs = _aligned_positive_probability(label_model_eval, X[val_idx])
        tree_cwe_probs = _aligned_cwe_probability(cwe_model_eval, X[val_idx][y_label[val_idx] == 1], len(cwe_classes))
        neural_label_probs, neural_cwe_probs = predict_multitask(
            neural_model,
            X_byte[val_idx],
            X_val_tab,
            batch_size=args.batch_size,
            device=DEVICE,
            desc="Final neural validation",
        )

        # Scalar fusion baselines (for reference)
        neural_label_weight, scalar_label_thresh, scalar_label_f1 = _search_binary_fusion(
            y_label[val_idx], tree_label_probs, neural_label_probs,
        )
        val_pos_mask = y_label[val_idx] == 1
        if val_pos_mask.any():
            neural_cwe_weight, scalar_cwe_macro = _search_cwe_fusion(
                y_cwe[val_idx][val_pos_mask],
                tree_cwe_probs,
                neural_cwe_probs[val_pos_mask],
            )
        else:
            neural_cwe_weight, scalar_cwe_macro = 1.0, 0.0

        print(f"scalar fusion baseline: label_f1={scalar_label_f1:.4f}, cwe_macro_f1={scalar_cwe_macro:.4f}")

        # MLP fusion meta-learner
        pipeline.set_postfix_str("MLP fusion")
        val_pos_full = y_label[val_idx] == 1
        tree_cwe_probs_full = np.zeros((len(val_idx), len(cwe_classes)), dtype=np.float32)
        tree_cwe_probs_full[val_pos_full] = tree_cwe_probs

        fusion_model, fusion_label_f1, fusion_cwe_macro, fusion_metrics = _train_fusion_mlp(
            tree_label_probs=tree_label_probs,
            tree_cwe_probs=tree_cwe_probs_full,
            neural_label_probs=neural_label_probs,
            neural_cwe_probs=neural_cwe_probs,
            y_label=y_label[val_idx],
            y_cwe=y_cwe[val_idx],
            num_cwe_classes=len(cwe_classes),
            epochs=args.fusion_epochs,
            batch_size=args.batch_size,
        )
        fused_label_f1 = fusion_metrics["label_f1"]
        fused_cwe_macro = fusion_metrics["cwe_macro_f1"]
        fusion_threshold = fusion_metrics["threshold"]
        print(f"MLP fusion: label_f1={fused_label_f1:.4f}, cwe_macro_f1={fused_cwe_macro:.4f}")
        pipeline.update(1)

        pipeline.set_postfix_str("save")
        torch.save(
            {
        ...
        torch.save(
            {
                "state_dict": {k: v.detach().cpu() for k, v in fusion_model.state_dict().items()},
                "num_cwe_classes": len(cwe_classes),
                "model_version": "v1.4",
            },
            MODEL_DIR / "fusion_mlp_v1.4.pt",
        )
        ...
        write_json(
            MODEL_DIR / FUSION_CONFIG_NAME,
            {
                ...
                # Scalar fusion (reference only)
                "scalar_tree_label_weight": float(1.0 - neural_label_weight),
                "scalar_neural_label_weight": float(neural_label_weight),
                "scalar_tree_cwe_weight": float(1.0 - neural_cwe_weight),
                "scalar_neural_cwe_weight": float(neural_cwe_weight),
                "scalar_label_f1": float(scalar_label_f1),
                "scalar_cwe_macro_f1": float(scalar_cwe_macro),
                # MLP fusion (active)
                "fusion_mode": "mlp",
                "fusion_mlp_model": "fusion_mlp_v1.4.pt",
        ...
        joblib.dump(
            {
                ...
                "cwe_classes": cwe_classes,
                "tree_threshold": float(tree_threshold),
                "tree_model_source": "trained_v1.4",
                "model_version": "v1.4",
            },
    ...
    print(f"saved fusion MLP to {MODEL_DIR / 'fusion_mlp_v1.4.pt'}")
