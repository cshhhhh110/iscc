"""v1.9 training: CosineAnnealingWarmRestarts + warmup + SWA + weighted ensemble."""
...
def main():
    ...
    seeds = args.seeds
    print(f"v1.9 ensemble training: seeds={seeds}, device={DEVICE}")
    ...
    joblib.dump({
        ...
        "model_version": "v1.9", "label_model_files": {s: seed_label_model(s) for s in seeds},
        "cwe_model_files": {s: seed_cwe_model(s) for s in seeds},
    }, MODEL_DIR / TABULAR_BUNDLE_NAME)

    print(f"\n{'='*60}")
    print(f"v1.9 training complete!")
    print(f"Fusion mode: scalar (weighted ensemble + SWA + cosine annealing)")
