...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=["all", "tree", "dcn", "tab_resnet", "pattern_lookup", "fusion"],
...
def resolve_arches(arch: str) -> list[str]:
    ...
    if arch in {"all", "fusion"}:
        return ["tree", "dcn", "tab_resnet", "pattern_lookup"]
    if arch in {"tree", "dcn", "tab_resnet", "pattern_lookup"}:
        return [arch]
...
def main() -> int:
    ...
    if not args.no_log:
        append_log(
            f"trained v1.3 PowerShell models; selected={selected_model_name}; "
            f"oof_macro_f1={report['oof_macro_f1']:.6f}; device={device}; bundle={model_output}"
        )
        append_total_log(
            f"powershell恶意脚本检测 v1.3; selected={selected_model_name}; oof_macro_f1={report['oof_macro_f1']:.6f}"
        )
