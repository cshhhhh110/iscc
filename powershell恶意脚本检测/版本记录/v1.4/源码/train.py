...
def train_sklearn_gbdt_cv(
    ...
    """5-fold CV → calibration → bundle for a single sklearn-compatible GBDT."""
    ...
    for fold, (train_idx, valid_idx) in enumerate(
        ...
        oof[valid_idx] = align_proba(model, model.predict_proba(X[valid_idx]), LABELS)
...
def main() -> int:
    ...
    if not args.no_log:
        append_log(
            f"trained {ARTIFACT_VERSION} PowerShell models; selected={selected_model_name}; "
            f"oof_macro_f1={report['oof_macro_f1']:.6f}; device={device}; bundle={model_output}"
        )
        append_total_log(
            f"powershell恶意脚本检测 {ARTIFACT_VERSION}; selected={selected_model_name}; oof_macro_f1={report['oof_macro_f1']:.6f}"
        )
