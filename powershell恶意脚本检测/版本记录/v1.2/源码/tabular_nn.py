...
def build_pattern_soft_targets(
    ...
    for label_idx in range(num_classes):
        if label_idx not in counts.columns:
            counts[label_idx] = 0
    counts = counts[sorted(counts.columns)]
    if list(counts.columns) != list(range(num_classes)):
    ...
    merged = frame[group_cols].merge(pattern_targets, on=group_cols, how="left", sort=False)
    ...
    entropy = -(pattern_targets[list(range(num_classes))].to_numpy(dtype=np.float32) * np.log(
        np.clip(pattern_targets[list(range(num_classes))].to_numpy(dtype=np.float32), 1e-12, 1.0)
    )).sum(axis=1)
