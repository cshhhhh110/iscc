...
def focal_loss(
    ...
    """Multi-class Focal Loss with soft targets.

    FL(p_t) = -(1 - p_t)^gamma * log(p_t)

    For soft targets, p_t = sum(target_probs * probs), i.e. the weighted
    probability assigned to the target distribution.
    """
    probs = torch.softmax(logits, dim=-1)
    p_t = (target_probs * probs).sum(dim=-1).clamp(min=1e-9)
    ...
    for epoch in epoch_iterator:
        ...
        for batch_features, batch_targets, _, batch_weights in loader:
            ...
            with torch.autocast(device_type=device.type, enabled=scaler.is_enabled()):
                logits = model(batch_features)
                if loss_type == "focal":
                    loss = focal_loss(logits, batch_targets, batch_weights, gamma=focal_gamma)
                else:
