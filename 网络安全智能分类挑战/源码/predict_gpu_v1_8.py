...
from gpu_v1_8_core import (
    VERSION, apply_robust_stats, build_model, log_key_metrics,
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description=f"Predict with {VERSION} FT+SWA bundle.")
...
def main() -> int:
    ...
    selected = bundle.get("selected", "swa")
    state_key = "swa_state" if selected == "swa" else "best_state"
    print(f"Using {state_key} ({selected})")
    ...
    for record in tqdm(fold_iter, desc=f"{VERSION} predict", unit="fold", dynamic_ncols=True):
        ...
        model.load_state_dict(record[state_key])
    ...
    append_action_log(args.action_log,
        f"{VERSION} predict done: selected={selected}, output={args.output_path}, folds={len(fold_iter)}.")

    log_key_metrics(root=data_dir, metrics={
        "version": VERSION, "stage": "predict", "model": f"ft_swa_{selected}",
        "n_features": len(feature_cols), "seeds": "-", "folds": len(fold_iter),
        "local_acc": "-", "local_macro_f1": "-", "weak_f1": "-",
        "platform_score": "-", "notes": f"submission={args.output_path.name},{selected}",
    })
