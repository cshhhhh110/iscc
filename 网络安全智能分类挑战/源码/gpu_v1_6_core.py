...
# Stage 1: remove perfectly correlated features (r>0.999)
DROP_REDUNDANT = [
...
# Stage 2: remove top drifted features (adversarial validation AUC=0.79→0.67)
DROP_DRIFTED = [
...
ALL_DROPS = DROP_REDUNDANT + DROP_DRIFTED


def select_features(feature_cols: list[str]) -> list[str]:
    return [c for c in feature_cols if c not in ALL_DROPS]
...
def fit_quantile_transformer(x: np.ndarray, seed: int) -> QuantileTransformer:
    qt = QuantileTransformer(
        n_quantiles=min(2000, len(x)), output_distribution="uniform",
...
def apply_quantile(x: np.ndarray, qt: QuantileTransformer) -> np.ndarray:
