...
# Features with pairwise correlation > 0.999 (perfectly redundant)
DROP_FEATURES = [
    "pattern_change_ratio",       # r=1.000 with pattern_transition_ratio
    "pattern_switch_frequency",   # r=1.000 with pattern_transition_ratio
    "pattern_mix_density",        # r=0.999 with pattern_diversity_ratio
]


def deduplicate_features(feature_cols: list[str]) -> list[str]:
    return [c for c in feature_cols if c not in DROP_FEATURES]


# ── FT-Transformer model (same architecture as v1.4) ──────────────────────
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn = nn.MultiheadAttention(
            embed_dim=d_token, num_heads=n_heads, dropout=attention_dropout, batch_first=True,
        )
        self.attn_dropout = nn.Dropout(residual_dropout)
        self.ffn_norm = nn.LayerNorm(d_token)
        self.ffn = nn.Sequential(
            nn.Linear(d_token, d_ffn * 2), GEGLU(), nn.Dropout(ffn_dropout), nn.Linear(d_ffn, d_token),
        )
...
class FTTransformerClassifier(nn.Module):
    def __init__(
        self, n_features: int, n_classes: int, d_token: int, n_blocks: int,
        n_heads: int, d_ffn: int, attention_dropout: float, residual_dropout: float,
        ffn_dropout: float, head_dropout: float,
    ) -> None:
        super().__init__()
...
def build_ft_transformer(n_features: int, n_classes: int, config: dict) -> nn.Module:
...
def predict_ft_probs(model: nn.Module, x: np.ndarray, device: torch.device,
...
def log_key_metrics(root: Path, metrics: dict) -> None:
    """Append a one-line JSON record to KEY_METRICS.md for easy iteration comparison."""
    import json
